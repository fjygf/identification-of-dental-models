	...
	ui(new Ui::Widget)
{
	...
	up_or_low = 1; //1-下，0-上
	char *filename2[2] = { "D:/牙齿/1上.stl", "D:/牙齿/1下.stl" };

	//window1 
	char *filename1[2] = { "D:/牙齿/1上all.stl", "D:/牙齿/1下all.stl" };
	...
	nameactor->GetTextProperty()->SetFontFile("D:/牙齿/FZSJ-WAMBJW.TTF");
...
//		textactor[i]->SetPosition((bounds[i][0] + bounds[i][0]) / 2, (bounds[i][2] + bounds[i][3]) / 2, (bounds[i][4] + bounds[i][5]) / 2);
	...
	nameactor1->GetTextProperty()->SetFontFile("D:/牙齿/FZSJ-WAMBJW.TTF");
